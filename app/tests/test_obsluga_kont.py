...
class TestObslugaKont(unittest.TestCase):
    ...
    def test_3_put(self):
        create_resp = requests.put(self.url + "/konta/aktualizuj_konto",json=self.body2)
        self.assertEqual(create_resp.status_code,201)
    def test_4_zmiana(self):
        get_resp = requests.get(self.url + f"/konta/konto/08321967998")
        self.assertEqual(get_resp.status_code,200)
        resp_body = get_resp.json()
    ...
    def test_5_usuwanie(self):
        create_resp = requests.delete(self.url + "/konta/usun_konto",json=self.body2)
        self.assertEqual(create_resp.status_code,201)